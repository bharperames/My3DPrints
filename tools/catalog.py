...
PARTS = [
    ...
    _p("knot", "The Knot", "Designed here", "parametric",
       ...
       "in a hex pocket in the next block around, each thread engages the "
       "block before, and the three blocks close into a solid cube with "
       "three bolt tips flush in three faces and the other three blank. "
       "Nothing protrudes and nothing visible moves. The weave is three "
       "skew axes cyclic under a third of a turn about the cube\u2019s "
       "diagonal, which makes every pair the same distance apart \u2014 so "
       "the whole object is one block and one bolt, each printed three "
       "times. Fully symmetric it is not a puzzle but a solid: every body "
       "has two bores at right angles, and sliding along either drags the "
       "other sideways across a shank, so nothing can translate at all, "
       "and what cannot come apart cannot go together either. The way in "
       "has to be built. One block\u2019s own bore is a plain slip fit "
       "rather than a thread, and its pocket is elongated along its own "
       "axis by exactly the depth of a head; that one block, and only that "
       "one, slides thirteen millimetres out of the cube, the bolt it "
       "uncovers is pushed further IN to lift its head clear of the "
       "keyway, and the cycle unzips from there. The size is derived from "
       "that slide rather than chosen: a pocket, its elongation and two "
       "walls have to fit along a block, which for this thread makes the "
       "cube 98 mm on a side, and every smaller version built along the "
       "way measured welded. Measured is the word. The depth of the "
       "puzzle is not argued from the drawing; a search sweeps every body "
       "along every axis under slide and both screw handednesses, from "
       "every configuration it can reach, and reports the number of legal "
       "first moves (one), the length of the shortest solution, and "
       "whether that solution needs a move in the wrong direction (it "
       "does). The same search was first shown to read the seed cube as "
       "two moves and a deliberately welded cube as zero, because an "
       "instrument that has not been made to read zero is not evidence. "
       "The thread is the seed cube\u2019s: the Montessori cosine, "
       "support-free at any scale printed along its axis, which is why the "
       "blocks stand with their threaded bores vertical and the pockets on "
       "their sides. PETG, not PLA \u2014 the seed cube\u2019s bolt "
       "snapped in silk PLA under hand torque, and this one is longer.",
       version="0.1.0",

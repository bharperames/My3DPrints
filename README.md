...
| `specimen/` | specimen meshes (glTF, millimetres, y up, base at y = 0) the orbital jig's capture page stands on its pedestal; `docs/orbital-jig.html?file=Name.glb` picks one, BigMeherrin.glb by default. Lifted from photographed outlines by FossilRecord's restoration pipeline, not scanned — the mesial-distal outline is measured, the labial/lingual thickness is a prior | no |
...
| `gen_chain.py` / `gen_cage.py` / `gen_dice_cage.py` / `gen_spiral.py` / `gen_clasp.py` / `gen_montessori.py` / `gen_orbital_jig.py` / `gen_tooth_stand.py` / `gen_specimen_heads.py` | on-demand generators; refuse to emit until verification passes (gen_spiral simulates the full screw-in path with FCL; gen_clasp checks flexure strain against PLA's elastic budget; gen_montessori casts its thread from the designer's own nut and proves it by screwing the real bolt in; gen_orbital_jig sweeps its camera carriage and rotor against every other body and writes the assembled poses into the 3MF, which `docs/orbital-jig.html` reads back to run the capture sequence on the printed geometry itself; `gen_tooth_stand` carries the specimen on four cones — two per root lobe, never in the notch — and gates its optical claim geometrically, that nothing rises above the plane of the four dish rims, having first shown with negative controls that a post beside the specimen occludes 8% and a 120 mm disc beneath it occludes nothing, so staying under that plane is the whole of it; `gen_specimen_heads` builds the screw-on alternative from the plan's section 6 so the two can be measured side by side) |
